...
select cat.name, count(*) as most_popular_film 
from inventory inv 
...
description, length as duration, rating
from film
where film_id 
...
from film as t2 where film_id not in (select film_id from inventory);
...
from payment inner join rental inner join inventory 
inner join film_actor inner join actor
...
select city.city, count(*) as no_of_films from inventory 
inner join rental inner join customer inner join address inner join city
on inventory.inventory_id = rental.inventory_id and rental.customer_id = customer.customer_id 
...
select concat_ws(" ", customer.first_name, customer.last_name) as customer, sum(payment.amount) as total_purchase 
from customer inner join payment inner join rental inner join inventory inner join film
on customer.customer_id = payment.customer_id 
and payment.rental_id = rental.rental_id and rental.inventory_id = inventory.inventory_id and inventory.film_id = film.film_id
...
from rental inner join customer inner join payment inner join inventory inner join film
    on rental.customer_id = customer.customer_id and rental.rental_id = payment.rental_id 
